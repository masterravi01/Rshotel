...
const getAmenityById = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, amenity, 'Amenity retrieved successfully')
    );
});

// POST create a new amenity
const createAmenity = asyncHandler(async (req, res) => {
    const { propertyUnitId, roomTypeId, amenityName, amenityCharges, amenityDescription } = req.body;
    ...
    return res.status(201).json(
        new ApiResponse(201, amenity, 'Amenity created successfully')
    );
});
...
const updateAmenityById = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { propertyUnitId, roomTypeId, amenityName, amenityCharges, amenityDescription } = req.body;
    const amenity = await RoomAmenity.findByIdAndUpdate(id, {
        propertyUnitId,
        roomTypeId,
        amenityName,
        amenityCharges,
    ...
    return res.status(200).json(
        new ApiResponse(200, amenity, 'Amenity updated successfully')
    );
});
...
const deleteAmenityById = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, { id }, 'Amenity deleted successfully')
    );
});
