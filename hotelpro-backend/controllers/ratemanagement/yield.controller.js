...
const getYieldById = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, yieldData, 'Yield data retrieved successfully')
    );
});
...
const createYield = asyncHandler(async (req, res) => {
    ...
    return res.status(201).json(
        new ApiResponse(201, yieldData, 'Yield data created successfully')
    );
});
...
const updateYieldById = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { date, ratePlanId, occupancy, revenue } = req.body;
    const yieldData = await Yield.findByIdAndUpdate(id, {
        date,
        ratePlanId,
        occupancy,
        revenue
    ...
    return res.status(200).json(
        new ApiResponse(200, yieldData, 'Yield data updated successfully')
    );
});
...
const deleteYieldById = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, { id }, 'Yield data deleted successfully')
    );
});
